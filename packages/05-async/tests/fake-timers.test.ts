...
describe("假定时器(fake timers)", () => {
  ...
  describe("Promise + fake timers(高频坑)", () => {
    it("retry 重试后成功", async () => {
      const task = vi.fn().mockRejectedValueOnce(new Error("fail1")).mockResolvedValueOnce("ok");
    ...
    it("runAllTimersAsync 跑光所有(含 Promise 链)", async () => {
      const task = vi.fn().mockRejectedValueOnce(new Error("boom1")).mockResolvedValueOnce("ok");
